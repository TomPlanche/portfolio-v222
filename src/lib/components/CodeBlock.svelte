...
<script lang="ts">
	...
	import { type CodeLine, dedent, highlight, languageLabel, parseLineSelection, tokenStyle } from '$lib/highlight';
...
<figure class={['code', className]}>
	...
	<div class="code__body">
		...
		<div
			...
			tabindex="0"
		>
			<!-- Whitespace is significant inside <pre>: every tag below is glued to
					 the next one on purpose, so the block renders the snippet and not the
					 indentation of this file. -->
			<!-- prettier-ignore -->
			<pre class="code__pre"><code class="code__lines">{#each rows as row (row.number)}<span class="code__line"
																																														 class:code__line--marked={row.marked}
																																														 class:code__line--dimmed={hasMarks && !row.marked}>{#if row.tokens}{#each row.tokens as token, index (index)}
				<span style={tokenStyle(token)}>{token.content}</span>{/each}{:else}{row.text}{/if}</span>{/each}</code></pre>
		</div>
	</div>
</figure>
